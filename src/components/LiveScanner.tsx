...
// --- CONFIGURATION (SENİN ONAYLADIĞIN AYARLAR) ---
const SCAN_STEPS = [
  { 
    id: 'front', 
    label: 'Front View', 
...
// --- CDN SCRIPT YÜKLEYİCİ (ÇÖKME ÖNLEYİCİ) ---
...
const LiveScanner: React.FC<LiveScannerProps> = ({ onComplete, onCancel }) => {
  ...
  // --- HESAPLAMA MANTIĞI (SENİN KODUN - GÜNCELLENDİ) ---
  ...
  // --- SONUÇ İŞLEME MANTIĞI (SENİN KODUN - GÜNCELLENDİ) ---
  const onResults = useCallback((results: any) => {
    if (!isMountedRef.current || status === 'capturing') return;
    if (!isModelLoaded) setIsModelLoaded(true); // Yüklendiğini bildir
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        // Scriptleri CDN'den çekiyoruz (En stabil yöntem)
        ...
        toast({ title: "Kamera Hatası", description: "Manuel moda geçildi.", variant: "destructive" });
      }
    };

    init();

    // 10 Saniye içinde açılmazsa manuel moda zorla
  ...
  const handleCapture = useCallback(() => {
    ...
    // Scale factor to keep image reasonable size
