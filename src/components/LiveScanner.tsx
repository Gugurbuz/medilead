...
// Eğer visionModel'i daha sonra kullanacaksan import et, şimdilik hata vermemesi için kapalı tutuyorum
// import { processHairImage } from '@/lib/visionModel';

// --- TİP TANIMLAMALARI (Global Window için) ---
declare global {
  interface Window {
    FaceMesh: any;
    Camera: any;
...
const LiveScanner: React.FC<LiveScannerProps> = ({ onComplete, onCancel }) => {
  ...
  // Refs
  const faceMeshRef = useRef<any>(null); // any kullandık çünkü CDN'den geliyor
  ...
  const speak = useCallback((text: string, force = false) => {
    ...
    // utterance.lang = 'en-US'; // İstersen İngilizce yapabilirsin
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // 3. Kamerayı Başlat (MediaPipe Camera Utils ile daha stabil)
        if (videoRef.current) {
    ...
    // 4. Güvenlik Zamanlayıcısı (7 saniye içinde açılmazsa manuel mod)
    const timer = setTimeout(() => {
        if (!isModelLoaded && isMountedRef.current) {
            console.warn("Zaman aşımı, manuel moda geçiliyor");
            setForceManualMode(true);
            setIsModelLoaded(true);
        }
    }, 7000);
  ...
  // --- HESAPLAMA (Senin verdiğin koddan) ---
  ...
  const onResults = useCallback((results: any) => {
    ...
    // Yüklendi işaretini ver
    if (!isModelLoaded) setIsModelLoaded(true);
    if (!currentStep) return;

    // 1. Işık Kontrolü (Basit Pixel Check)
    // MediaPipe results.image canvas değil ImageBitmap dönebilir, o yüzden videoRef'ten bakmak daha güvenli olabilir ama
    // şimdilik performans için atlıyoruz veya basit bir kontrol ekleyebiliriz.
    ...
    if (currentStep.guideType === 'manual' || forceManualMode) {
         // Yüz varsa veya zorla manuel moddaysa
         if (forceManualMode) {
             setStatus('locked'); 
         } else if (!hasFace) { 
             // Arkası dönük (yüz yok) -> Doğru
             setStatus('locked');
             setScanProgress(prev => Math.min(prev + 1.5, 100));
         } else {
             // Yüz var -> Yanlış (Arkası dönük olmalı)
             setScanProgress(0);
             setStatus('aligning');
         }
         return;
  ...
  return (
    ...
    >
      ...
      <div className="relative flex-1 bg-gray-900 overflow-hidden flex items-center justify-center">
        ...
        {/* AR KONTROLLER (Sadece Ön Kamera) */}
